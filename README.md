...
- ' ,' 등의 경우 예외처리
### 2. 첫 번째 확인 버튼 클릭 이벤트
- 1. 의 결과에서 나온 배열을 바탕으로 `Car`객체 생성
- 시도할 횟수 입력받을 컴포넌트 생성
- 확인 버튼 비활성화
### 3. 사용자로부터 시도할 횟수 입력
- 공백이 있는지 확인
...
- '-' ,'1e9' 등 예외처리
### 4. 두 번째 확인 버튼 클릭 이벤트
- 각각의 Car 객체에 무작위 난수 생성
- 확인 버튼 비활성화
### 5. Car 객체 갱신
- 난수가 4 이상인 경우 `move` +1
- 난수가 3 이하인 경우 `move` 그대로
- 입력받은 시도 횟수만큼 반복
### 6. 결과 종합
- `Car`객체의 `move`에 따라서 배열 정렬
- 가장 `move`가 큰 객체와 같은 `move`를 가진 `Car`객체 파악
- 우승자 출력 (',' 로 구분)


<details><summary>주어진 요구사항</summary>
